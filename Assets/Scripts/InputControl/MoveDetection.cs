...
public class MoveDetection : MonoBehaviour
{
    ...
    float _timeThreshold = 1f;
    float _distanceMultiplier = 0.01f;

    IDictionary<SIDE, float> borders = new Dictionary<SIDE, float>();

    private void Awake()
    {
        _timeThreshold = config.TimeThreshold;
        _distanceMultiplier = config.DistanceMultiplier;
        ...
        detectionTimer = new WaitForSeconds(_timeThreshold);
        pauseUpdate = true;
        noPrevPosition = true;
    ...
    void Move(Vector2 positionDelta, Vector2 direction)
    {
        ...
        {
            Vector2 delta = CalculateDeltaToTranslate(new Vector2(-positionDelta.x * _distanceMultiplier, -positionDelta.y * _distanceMultiplier));
