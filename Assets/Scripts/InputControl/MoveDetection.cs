...
public class MoveDetection : MonoBehaviour
{
    ...
    void Move(Vector2 positionDelta, Vector2 direction)
    {
        ...
        if (cameraTransform.position.y + cameraHeight / 2 + 0.01f < borders[SIDE.up] ||
            cameraTransform.position.y - cameraHeight / 2 - 0.01f > borders[SIDE.down] ||
            cameraTransform.position.x - cameraWidth / 2 - 0.01f > borders[SIDE.left] ||
            cameraTransform.position.x + cameraWidth / 2 + 0.01f < borders[SIDE.right])
        {
