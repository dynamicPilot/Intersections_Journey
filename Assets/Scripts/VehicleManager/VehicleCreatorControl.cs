...
public class VehicleCreatorControl : MonoBehaviour
{
    ...
    //IDictionary<int, List<bool>> timetableForStartPoints = new Dictionary<int, List<bool>>();
    ...
    {
        //int minuteInteval = Mathf.FloorToInt(timeControl.GameMinutesFerRealSecond * timeControl.SecondBetweenTimeUpdate);      
        
        // form all structures

        //for (int i = 0; i < 60; i += minuteInteval) minuteIntervals.Add(i);
            ...
            foreach (TimeInterval timeInterval in timeIntervalsByRoad.TimeIntervals)
            {
                ...
                foreach (VehicleNumberByType vehicleNumber in timeInterval.VehicleNumbersList)
                {
                    for (int i = 0; i < vehicleNumber.number; i++)
                    {
                        ...
                        timetable[hour][minuteIntervals[minuteIndex]].Add(new VehicleNumberByType(vehicleNumber.type, timeIntervalsByRoad.RoadStartPointNumber, vehicleNumber.needSpecialEndPoint, vehicleNumber.endPointNumber));
                    }
                }
            }
        }
