...
public class RoadTriggersControl : MonoBehaviour
{
    ...
    public void TriggerIsTriggered(int triggerStartPointNumber, float totalTimeOnRoad = 0, TYPE vehicleType = TYPE.car)
    {
        ...
        if (!isRoadEnd)
        {
            ...
            roadsManager.RoadTriggersOnEnter(triggerStartPointNumber);
        }
        else
        {
            roadsManager.RemoveVehicleFromRoad(triggerStartPointNumber, totalTimeOnRoad, vehicleType);
        }
    }
}
